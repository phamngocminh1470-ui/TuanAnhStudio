# -*- coding: utf-8 -*-
"""
Script xây dựng mới toàn diện component TeacherPortal.jsx
Bao gồm:
1. Màn hình khóa bảo mật & kích hoạt giáo viên (Liên hệ Zalo: 0975711254 - Admin Tuấn Anh)
2. Không gian quản trị lớp học riêng tư (Private Classroom Workspace: Overview, Assignments, Gradebook, Announcements, Vocab & Essay)
3. Thử thách tuần toàn diện: Từ vựng + Đặt câu + Viết đoạn văn/Bài luận (AI chấm 4 tiêu chí THPT)
4. Công cụ xáo đề thi 101-104 tự động từ đề của giáo viên
"""
...
import { 
  ...
  UserCheck, AlertTriangle, FileSpreadsheet, ExternalLink
} from 'lucide-react';
...
// Mẫu thử thách tuần (Bao gồm Cả Từ Vựng + Đặt Câu + Viết Đoạn Văn / Bài Luận)
const INITIAL_TOPICS = [
  {
    ...
    submissions: [
      ...
      { 
        ...
        sentence: 'Deforestation causes severe soil erosion and destroys animal habitats.', 
        essay: '',
        aiScore: 9.0, 
        aiFeedback: 'Cấu trúc câu chính xác, liên kết ý logic.', 
        type: 'vocab',
        status: 'approved' 
      }
    ]
...
// Danh sách lớp học mẫu với dữ liệu không gian riêng
const INITIAL_CLASSES = [
  { 
    ...
    assignments: [
      { id: 'asg-1', title: 'Bài Tập Trắc Nghiệm Unit 1: Family Life (20 Câu)', deadline: '2026-09-10', submittedCount: 35, totalStudents: 38, status: 'open' },
      { id: 'asg-2', title: 'Đề Thi Thử Khảo Sát 40 Câu Chuẩn Cấu Trúc THPT 2026', deadline: '2026-09-18', submittedCount: 28, totalStudents: 38, status: 'open' }
    ],
    students: [
  ...
  { 
    ...
    activeAssignments: 3,
    announcements: [
      { id: 'ann-3', title: 'Luyện đề thích ứng IRT cuối tuần', content: 'Các em vào tab Luyện Đề Thích Ứng làm bài kiểm tra để hệ thống đo năng lực Theta cá nhân nhé.', date: '2026-08-26' }
    ],
    assignments: [
      { id: 'asg-3', title: 'Đề Khảo Sát Sở GD&ĐT Hà Nội Lớp 11', deadline: '2026-09-12', submittedCount: 40, totalStudents: 42, status: 'open' }
    ],
    students: [
  ...
  { 
    ...
    activeAssignments: 4,
    announcements: [
      { id: 'ann-4', title: 'Bộ 40 Câu Đề Minh Họa Chuẩn Bộ GD&ĐT 2026', content: 'Đề thi đã được mở kèm đáp án và lời giải chi tiết. Yêu cầu 100% các bạn hoàn thành trước ngày 15/09.', date: '2026-08-27' }
    ],
    assignments: [
      { id: 'asg-4', title: 'Đề Minh Họa Tốt Nghiệp THPT 2026 (40 Câu Chuẩn Bộ GD&ĐT)', deadline: '2026-09-15', submittedCount: 43, totalStudents: 45, status: 'open' }
    ],
    students: [
...
export default function TeacherPortal({ keys, onNavigate }) {
  // ════════════════════════════════════════════════════════════════════════════
  // 0. BẢO MẬT & KÍCH HOẠT QUYỀN GIÁO VIÊN (LIÊN HỆ ZALO: 0975711254)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // Xử lý nhập mã kích hoạt giáo viên
  ...
  const handleSendTeacherRegistration = (e) => {
    ...
    setTimeout(() => {
      // Tự động cấp quyền kích hoạt ngay sau khi gửi thông tin
      setIsTeacherActivated(true);
      localStorage.setItem('is_teacher_activated', 'true');
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // 1. TÍNH NĂNG XÁO ĐỀ THI TRẮC NGHIỆM (101 - 104)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // Thuật toán xáo trộn câu hỏi và đáp án
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // 2. TÍNH NĂNG QUẢN LÝ LỚP HỌC & KHÔNG GIAN RIÊNG TƯ (WORKSPACE)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  const [classWorkspaceTab, setClassWorkspaceTab] = useState('overview'); // 'overview' | 'assignments' | 'gradebook' | 'announcements' | 'vocab_essay'
  ...
  const handleAddAssignment = () => {
    ...
    const updatedClass = {
      ...
      activeAssignments: selectedClassWorkspace.activeAssignments + 1
    };
    ...
    alert("✓ Đã giao bài tập mới cho lớp thành công!");
  };
  ...
  // Xuất sổ điểm lớp ra file CSV / Excel
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // 3. TÍNH NĂNG THỬ THÁCH TUẦN: TỪ VỰNG + ĐẶT CÂU + VIẾT ĐOẠN VĂN / BÀI LUẬN
  // ════════════════════════════════════════════════════════════════════════════
...
Trả về định dạng JSON thuần túy:
{
  "score": 9.0,
  "is_correct": true,
  "grammar_analysis": "Phân tích ngữ pháp ngắn gọn",
  "improved_sentence": "Phiên bản nâng cấp chuẩn Band 8.0+",
  "feedback": "Lời khuyên"
}`;
      let aiResponse = {
        score: 9.0,
        ...
        improved_sentence: submitSentence ? `${submitSentence} (Polished)` : submitWord,
        feedback: "Từ vựng được áp dụng rất tốt!"
      };
  ...
  // AI Chấm Bài Viết Đoạn Văn / Bài Luận Theo 4 Tiêu Chí THPT
...
{
  "task_response_score": 2.3,
  "coherence_score": 2.4,
  "lexical_score": 2.3,
  "grammar_score": 2.2,
  "total_score": 9.2,
  "detailed_feedback": "Nhận xét tổng quát về bố cục, từ nối và từ vựng",
  "corrections": "Chỉ ra 1-2 câu cần sửa lỗi ngữ pháp hoặc diễn đạt tự nhiên hơn",
  "band9_model": "Phiên bản đoạn văn nâng cấp chuẩn Band 9.0+ mẫu"
}`;
      ...
      if (keys?.gemini) {
        ...
        if (res.data?.response) {
          try {
            ...
            aiFeedback = `${parsed.detailed_feedback} • Sửa lỗi: ${parsed.corrections}`;
          } catch (e) {}
        }
      }
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // 4. MÀN HÌNH KHÓA BẢO MẬT KHI CHƯA KÍCH HOẠT QUYỀN GIÁO VIÊN
  // ════════════════════════════════════════════════════════════════════════════
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 py-10 px-4 animate-fade-in">
        ...
        <div className="glass-card rounded-3xl p-8 md:p-12 border border-amber-500/30 bg-gradient-to-b from-[#161208] via-[#0e0c1a] to-[#070b18] text-center space-y-8 shadow-2xl relative overflow-hidden">
          ...
          {/* Hỗ trợ kích hoạt qua Zalo / Hotline */}
            ...
            <div className="glass-card w-full max-w-lg rounded-3xl p-6 md:p-8 border border-cyan-500/40 bg-[#090e24] space-y-5 shadow-2xl relative">
              ...
              {contactSuccess ? (
                <div className="p-6 text-center space-y-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl">
                  ...
                  <p className="text-xs text-gray-300">Hệ thống đang chuyển Thầy/Cô vào giao diện quản trị...</p>
                </div>
              ) : (
                <form onSubmit={handleSendTeacherRegistration} className="space-y-3.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="text-[11px] text-gray-300 font-bold block mb-1">Trường THPT Đang Giảng Dạy:</label>
                      <input
                        type="text"
                        value={teacherSchool}
                  ...
                  <div className="pt-2 flex justify-end gap-2">
                    ...
                    <button
                      type="submit"
                      className="px-5 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-extrabold shadow-lg shadow-cyan-500/20"
                    >
                      Gửi &amp; Kích Hoạt Ngay
                    </button>
                  </div>
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // 5. GIAO DIỆN CHÍNH CỦA GIÁO VIÊN KHI ĐÃ ĐƯỢC KÍCH HOẠT
  // ════════════════════════════════════════════════════════════════════════════
  return (
    <div className="space-y-8 w-full max-w-[1600px] mx-auto px-4 md:px-8 pb-20 animate-fade-in">
      
      {/* Top Banner với Trạng thái Xác thực & Đổi Tab */}
      <div className="glass-card rounded-3xl p-6 md:p-8 border border-white/10 bg-gradient-to-r from-[#0c1430] via-[#070b1a] to-[#120a28] shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs md:text-sm text-slate-300 max-w-2xl font-normal">
            Không gian riêng tư để Thầy/Cô quản lý lớp học, xáo đề thi trắc nghiệm (101-104), giao bài tập định kỳ và quản lý thử thách Viết đoạn văn / Từ vựng tuần.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-red-500/20 text-slate-400 hover:text-red-300 text-xs font-bold transition border border-white/10 flex items-center gap-1.5 cursor-pointer"
      ...
      <div className="flex items-center p-1.5 rounded-2xl bg-white/[0.04] border border-white/10 w-fit overflow-x-auto no-scrollbar gap-1">
        <button
          onClick={() => { setActiveSection('classes'); setSelectedClassWorkspace(null); }}
        ...
        >
          ...
          <span>Quản Lý Lớp Học &amp; Không Gian Riêng ({classes.length} Lớp)</span>
        </button>

        <button
          onClick={() => { setActiveSection('weekly-topic'); setSelectedClassWorkspace(null); }}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-extrabold transition cursor-pointer ${
        ...
        <button
          onClick={() => { setActiveSection('shuffler'); setSelectedClassWorkspace(null); }}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-extrabold transition cursor-pointer ${
        ...
        >
          ...
          <span>Công Cụ Xáo Đề Thi (Mã 101 - 104)</span>
        </button>
      </div>

      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/* SECTION 1: QUẢN LÝ LỚP HỌC & KHÔNG GIAN RIÊNG TƯ (WORKSPACE DASHBOARD)  */}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {activeSection === 'classes' && (
        <div className="space-y-6">
          
          {/* A. KHÔNG GIAN QUẢN TRỊ LỚP HỌC RIÊNG BIỆT KHI ĐÃ CHỌN 1 LỚP */}
          {selectedClassWorkspace ? (
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  ...
                  <div className="bg-black/50 p-4 rounded-2xl border border-amber-500/30 flex items-center gap-4 shrink-0">
                    ...
                    >
                      ...
                      <span>{copiedKey ? 'Đã Copy!' : 'Copy Mã'}</span>
                    </button>
                  </div>
                </div>
              ...
              {classWorkspaceTab === 'overview' && (
                <div className="space-y-6">
                  ...
                  {/* Bảng Học Sinh Cần Chú Ý */}
                  <div className="glass p-6 rounded-2xl border border-white/10 space-y-4">
                    <h3 className="font-extrabold text-sm text-white flex items-center gap-2">
                      <AlertTriangle className="w-4 h-4 text-amber-400" />
                      <span>Học Sinh Cần Phụ Đạo &amp; Đánh Giá Năng Lực</span>
                    </h3>
                    <div className="divide-y divide-white/5 border border-white/5 rounded-xl overflow-hidden">
                      {(selectedClassWorkspace.students || []).map((st, idx) => (
                        <div key={st.id || idx} className="p-3.5 bg-black/30 flex items-center justify-between text-xs">
                          <div className="flex items-center gap-3">
                            <span className="font-mono text-gray-500 w-5">{idx + 1}.</span>
                            <span className="font-bold text-white">{st.name}</span>
                            <span className="text-gray-400 text-[11px]">({st.email || 'Học sinh'})</span>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="font-bold font-mono text-slate-300">Điểm TB: <strong className={st.avg >= 8 ? 'text-emerald-400' : st.avg >= 5 ? 'text-amber-400' : 'text-red-400'}>{st.avg}</strong></span>
                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                              ...
                              {st.status === 'completed' ? '✓ Tốt' : st.status === 'needs_help' ? '⚠️ Cần Phụ Đạo' : 'Đang Làm'}
                            </span>
                          </div>
                        </div>
                      ))}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">Giao đề thi trắc nghiệm hoặc bài kiểm tra định kỳ cho toàn bộ học sinh trong lớp.</p>
                    </div>
                    <button
                  ...
                  {isCreatingAssignment && (
                    <div className="glass p-5 rounded-2xl border border-indigo-500/30 bg-indigo-950/20 space-y-3 animate-in fade-in">
                      <h4 className="font-bold text-sm text-indigo-300">Tạo &amp; Giao Bài Tập Mới</h4>
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="sm:col-span-2">
                          <label className="text-[11px] text-gray-300 font-bold block mb-1">Tiêu Đề Bài Tập / Đề Thi:</label>
                          <input
                            type="text"
                            value={newAssignmentTitle}
                            onChange={(e) => setNewAssignmentTitle(e.target.value)}
                            placeholder="Ví dụ: Đề Ôn Tập 40 Câu Trắc Nghiệm Unit 3..."
                            className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none"
                          />
                        </div>
                        <div>
                          <label className="text-[11px] text-gray-300 font-bold block mb-1">Hạn Nộp (Deadline):</label>
                          <input
                            type="date"
                      ...
                      <div className="flex justify-end gap-2 pt-2">
                        ...
                        <button onClick={handleAddAssignment} className="px-5 py-2 rounded-xl bg-indigo-600 text-white text-xs font-bold shadow">Xác Nhận Giao Cho Lớp</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(selectedClassWorkspace.assignments || []).map(asg => (
                      <div key={asg.id} className="glass p-5 rounded-2xl border border-white/10 space-y-3">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-400 pt-2 border-t border-white/5">
                          ...
                          <span>Đã nộp: <strong className="text-emerald-400">{asg.submittedCount} / {asg.totalStudents}</strong></span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            /* B. DANH SÁCH TẤT CẢ LỚP HỌC (VIEW TỔNG QUAN) */
            <div className="space-y-6 animate-fade-in">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    Bấm vào từng lớp để mở <strong>Không Gian Quản Trị Riêng Tư</strong> (giao bài tập, xem sổ điểm, đăng thông báo).
                  </p>
                </div>
              ...
              {/* Danh sách các Lớp học (Card Click mở Workspace) */}
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/* SECTION 2: THỬ THÁCH TUẦN: TỪ VỰNG + ĐẶT CÂU + VIẾT ĐOẠN VĂN / BÀI LUẬN*/}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {activeSection === 'weekly-topic' && (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="space-y-6">
            {topics.map((top) => (
              <div key={top.id} className="glass p-5 md:p-7 rounded-3xl border border-white/10 space-y-6">
                ...
                {/* KHU VỰC THỰC HÀNH NỘP BÀI CỦA HỌC SINH */}
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-2">
                  
                  {/* Cột 1: Nộp Từ Vựng & Đặt Câu Nhanh */}
                  ...
                  {/* Cột 2: Nộp Đoạn Văn / Bài Luận Hoàn Chỉnh */}
      ...
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {/* SECTION 3: CÔNG CỤ XÁO ĐỀ THI TRẮC NGHIỆM TỰ ĐỘNG (101 - 104)          */}
      {/* ═══════════════════════════════════════════════════════════════════════ */}
      {activeSection === 'shuffler' && (
        <div className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-extrabold text-white font-outfit flex items-center gap-2">
                ...
                <span>Công Cụ Xáo Đề Thi Trắc Nghiệm Tự Động (Mã 101 - 104)</span>
              </h2>
              <p className="text-xs text-gray-400 mt-0.5">
                Giáo viên chỉ cần dán 1 đề thi gốc. Hệ thống tự động đảo ngẫu nhiên câu hỏi và các phương án A/B/C/D để tạo 2, 4, 6 hoặc 8 mã đề riêng biệt kèm bảng đáp án.
              </p>
            </div>

            <button
              onClick={handleShuffleExam}
              className="px-6 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-extrabold text-xs shadow-lg shadow-orange-500/25 transition cursor-pointer flex items-center gap-2"
            >
              <Shuffle className="w-4 h-4" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {/* Cột Phải: Xem Mã Đề Đã Sinh & Ma Trận Đáp Án */}
            <div className="lg:col-span-7 space-y-4">
              {shuffledExams ? (
                <div className="glass p-5 rounded-2xl border border-white/10 space-y-5">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-gray-400">Chọn mã đề xem trước:</span>
                    ...
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer border border-white/10"
                    >
                      <Copy className="w-3.5 h-3.5 text-amber-400" />
                      <span>{copiedKey ? 'Đã Copy Đề!' : 'Copy Đề Này'}</span>
                    </button>
                  </div>
                ...
                <div className="glass p-12 rounded-2xl border border-white/10 text-center space-y-4 flex flex-col items-center justify-center min-h-[350px]">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1 max-w-md">
                      Dán đề thi của Thầy/Cô ở cột bên trái và bấm <strong>"Xáo Đề &amp; Sinh Mã Đề"</strong> để tạo 4 mã đề kèm đáp án tự động!
                    </p>
                  </div>
                </div>
              )}
...
print("TeacherPortal.jsx successfully upgraded with security gate, private workspace and essay challenges!")
