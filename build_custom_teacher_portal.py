...
// Hàm phân tích văn bản đề thi của giáo viên thành danh sách câu hỏi
...
export default function TeacherPortal({ keys, onNavigate }) {
    ...
    for (let i = 0; i < numVariants; i++) {
      ...
      const finalQuestions = shuffledQList.map((q, qIdx) => {
        ...
        return {
          ...
          originalId: q.id,
          questionText: q.question.replace(/^(Câu\s+\d+:|Question\s+\d+:)/i, `Câu ${qIdx + 1}:`),
  ...
  // Xuất file Word (.doc) chuẩn UTF-8
...
`;
    ...
    html += `<h3 style="text-align: center; font-weight: bold;">BẢNG ĐÁP ÁN MÃ ĐỀ ${exam.examCode}</h3>`;
  ...
  // Xuất trọn bộ file Word kèm ma trận đáp án
  const handleExportAllWord = () => {
    if (!shuffledExams || shuffledExams.length === 0) return;
    let html = `<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
<head><meta charset='utf-8'><title>TRỌN BỘ ĐỀ THI TIẾNG ANH THPT</title>
<style>
body { font-family: 'Times New Roman', serif; font-size: 13pt; line-height: 1.4; color: #000; }
.header { width: 100%; margin-bottom: 20px; border-bottom: 2px solid #000; padding-bottom: 10px; }
.title { text-align: center; font-weight: bold; font-size: 15pt; text-transform: uppercase; margin: 10px 0; }
...
`;
  ...
  // Đọc file từ máy tính giáo viên tải lên (.txt / .doc)
  const handleFileUploadExam = (e) => {
    ...
    reader.onload = (event) => {
      ...
      setNewAssignmentText(content);
      const parsed = parseExamTextToQuestions(content);
      alert(`✓ Đã tải file lên thành công! Hệ thống nhận diện được ${parsed.length} câu hỏi trắc nghiệm.`);
    };
    reader.readAsText(file, 'UTF-8');
  };

  // Giáo viên xác nhận giao bài tập mới
  ...
  // Mở màn hình làm bài trực tuyến
      ...
      <div className="glass-card rounded-3xl p-6 md:p-8 border border-white/10 bg-gradient-to-r from-[#0c1430] via-[#070b1a] to-[#120a28] shadow-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-xs md:text-sm text-slate-300 max-w-2xl font-normal">
            Không gian riêng để Thầy/Cô quản lý lớp học, tự tạo &amp; tải đề kiểm tra lên giao cho học sinh làm trực tuyến, xáo đề trắc nghiệm và xuất file Word / PDF.
          </p>
        </div>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              {classWorkspaceTab === 'assignments' && (
                <div className="space-y-6">
                    ...
                    <div className="glass p-6 rounded-2xl border border-indigo-500/40 bg-indigo-950/30 space-y-4 animate-in fade-in">
                      <div className="flex flex-wrap items-center justify-between border-b border-white/10 pb-3 gap-2">
                        ...
                        <label className="px-3.5 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-bold border border-white/20 transition flex items-center gap-1.5 cursor-pointer">
                          ...
                          <span>Tải File Đề Từ Máy Tính (.txt / .doc)</span>
                          <input type="file" accept=".txt,.doc,.docx" onChange={handleFileUploadExam} className="hidden" />
                        </label>
                      </div>
                      ...
                      {/* Khung Dán Nội Dung Đề Thi Của Giáo Viên */}
                      <div>
                  ...
                  {/* Danh sách bài tập do giáo viên tạo */}
